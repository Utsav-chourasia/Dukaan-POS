/* ============================================================
   orders.js — Order lifecycle with decimal quantity support
   ============================================================ */
...
function createOrder() {
  const order = {
    id: DB.nextOrderId++, items: [],
    customerId: null, finalized: false, payment: null,
    created: new Date().toISOString(), finalizedAt: null,
  };
...
/* ── Add product with DECIMAL qty support ───────────── */
function addProductToOrder(product, qty = 1) {
  ...
  // Round to 3dp to avoid floating-point accumulation
  ...
  toast(`${product.name} × ${fmtQty(qRounded)} ${product.unit}`);
}

/* ── Change qty (±0.25 for weight items, ±1 for others) */
function changeItemQty(itemIndex, delta) {
  const order = currentOrder();
  if (!order || order.finalized) return;
  const item = order.items[itemIndex];
  const step = ['kg','g','litre','liter','l'].includes(item.unit.toLowerCase()) ? 0.25 : 1;
...
/* ── Tap qty to edit inline ─────────────────────────── */
function editItemQty(itemIndex) {
  const order = currentOrder();
  if (!order || order.finalized) return;
  const item = order.items[itemIndex];
  openModal('editqty', { itemIndex, item });
}
...
function finalizeOrder(payment = 'paid') {
  ...
  toast(`#${order.id} ✓  ₹${fmtPrice(total)} · ${payment === 'paid' ? 'Paid' : 'Due'}`);
}
...
function _orderTabsHTML() {
  if (!DB.orders.length) return '<div style="padding:8px 2px;font-size:12px;color:var(--muted)">No active orders</div>';
  return DB.orders.map(o => {
    const cust = o.customerId ? findCustomer(o.customerId) : null;
    return `<div class="order-tab ${o.id===activeOrderId?'active':''}" onclick="switchOrder(${o.id})">
      <div class="ot-id">#${o.id} <span style="font-weight:400;color:var(--muted);font-size:10px">${o.finalized?'✓':'open'}</span></div>
      <div class="ot-meta">${cust?escHtml(cust.name):'Walk-in'} · ₹${fmtPrice(orderTotal(o))}</div>
    </div>`;
  }).join('');
}
...
function renderCurrentOrder() {
  ...
  if (!order) {
    ...
    setHTML('order-items', '<div class="empty-state"><span class="empty-icon">🛒</span>Tap "+ Order" to begin billing</div>');
    totalEl.textContent = '₹0'; countEl.textContent = '';
  ...
  titleEl.textContent = `#${order.id} — ${cust ? cust.name : 'Walk-in'}${order.finalized ? ' ✓' : ''}`;
  ...
  if (!order.items.length) {
    itemsEl.innerHTML = '<div class="empty-state"><span class="empty-icon">🔍</span>Search above or use 🎤 mic<br><small>Try: "cheeni", "0.5 kg chawal"</small></div>';
    return;
  }
  ...
  itemsEl.innerHTML = order.items.map((item, idx) => {
    ...
    return `<div class="order-item ${item.delivered?'delivered':''}" id="oi-${idx}">
      <label class="item-label">
        <input type="checkbox" class="delivered-check" ${item.delivered?'checked':''} ${locked?'disabled':''} onchange="toggleItemDelivered(${idx},this.checked)">
        <div>
      ...
      <div class="item-controls">
        ...
        <span class="qty-val" onclick="${locked?'':''}" title="Tap to edit" ${locked?'':` onclick="editItemQty(${idx})"`}>${fmtQty(item.qty)}</span>
        ${locked ? '' : `<button class="qty-btn" onclick="changeItemQty(${idx},1)">+</button>`}
...
function searchProducts(query) {
  ...
  if (!results.length) {
    el.innerHTML = `<div style="font-size:12px;color:var(--muted);padding:4px">
      No product — <a href="#" onclick="openModal('product');return false" style="color:var(--accent)">Add "${escHtml(q)}"?</a>
    </div>`;
    return;
  }
  ...
  el.innerHTML = results.map(p => {
    ...
    return `<div class="prod-chip ${zero?'out-of-stock':''}" onclick="addProductToOrder(findProduct(${p.id}),1)" title="${escAttr(p.aliases.join(', '))}">
      <span class="pc-name">${escHtml(p.name)}</span>
      <span class="pc-detail ${low?'low':''}">₹${p.price}/${p.unit} · ${zero?'Out':fmtQty(p.stock)+' left'+(low?' ⚠':'')}</span>
    </div>`;
  }).join('');
}
